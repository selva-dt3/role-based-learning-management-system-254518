...
- Env: copy `.env.example` to `.env` and set `REACT_APP_API_BASE_URL` (and `REACT_APP_USE_MOCK_API=true` to use the in-browser mock API).
  - For the running backend in this workspace, use:
    ```
...
- Frontend checks GET /employees/{id} against the configured base URL before showing Employee assignments.
- HR/Admin can POST /employees to create employee profiles.
- Lessons endpoints use plural routes: POST /lessons, PUT /lessons/{id}, DELETE /lessons/{id}.